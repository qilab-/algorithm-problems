...
class P03Spec extends FunSpec with Matchers {
  ...
  describe("nth2(Int, List)") {
    it("(*) Find the Kth element of a list. By convention, the first element in the list is element 0.") {
      val list = List(1, 1, 2, 3, 5, 8)
      list.zipWithIndex.foreach { case (elem, index) =>
        P03.nth2(index, list) should === (elem)
      }
    }

    it("throws NoSuchElementException if n >= size of the list.") {
      intercept[NoSuchElementException](P03.nth2(0, List.empty[Int]))
      intercept[NoSuchElementException](P03.nth2(1, List(1)))
      intercept[NoSuchElementException](P03.nth2(2, List(1)))
    }

    it("throws NoSuchElementException if n < 0.") {
      intercept[NoSuchElementException](P03.nth2(-1, List(1)))
      intercept[NoSuchElementException](P03.nth2(-2, List(1)))
    }
  }
